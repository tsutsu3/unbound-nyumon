...
/** 本文で使ってよい Directive。注意書きの役割は 4 種に固定されている。 */
...
export function parseOpening(line) {
  ...
  if (rest.startsWith("{")) {
    const end = matchingBracket(rest, 0, "{", "}");
    if (end === -1)
      return { colons: colonRun.length, name, title, attributes, valid: false };
...
export function findDirectives(body) {
  ...
  const close = (depthIndex, endLine) => {
    ...
    for (let k = stack.length - 1; k >= depthIndex; k--) {
      ...
      if (k > depthIndex) {
        ...
        entry.problems.push(
          `The closing line of \`${":".repeat(entry.colons)}${entry.name}\` closes the outer \`${":".repeat(outer.colons)}${outer.name}\` first. Add colons to the outer directive when nesting (\`${":".repeat(entry.colons + 1)}${outer.name}\`).`,
        );
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const kind =
      opening.name === COLUMN_NAME
        ? "column"
        : ASIDE_NAMES.has(opening.name)
          ? "aside"
          : "unknown";
    ...
    const entry = {
      ...
      title,
      version:
        kind === "column" ? COLUMN_VERSION_PATTERN.exec(title)?.[1] : undefined,
    ...
    if (!opening.valid) {
      entry.problems.push(
        `Unrecognized text after \`${line.trim()}\`.`,
      );
    }
    found.push(entry);
 ...
 * 日本語主体の本文なので、単語数ではなく文字数を目安にする (400〜800 字)。
 */
